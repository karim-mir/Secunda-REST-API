...
class ModelTests(TestCase):
    def setUp(self):
        ...
        self.building = Building.objects.create(
            address="г. Москва, ул. Тестовая 1", latitude=55.7558, longitude=37.6173
        )

        self.parent_activity = Activity.objects.create(name="Еда")
        self.child_activity = Activity.objects.create(
            name="Мясная продукция", parent=self.parent_activity
        )
...
class APITests(APITestCase):
    def setUp(self):
        self.building = Building.objects.create(
            address="г. Москва, ул. API Тест 1", latitude=55.7558, longitude=37.6173
        )

        self.parent_activity = Activity.objects.create(name="Еда")
        self.child_activity = Activity.objects.create(
            name="Молочная продукция", parent=self.parent_activity
        )
    ...
    def test_organizations_list_with_valid_api_key(self):
        ...
        if "results" in response.data:
            ...
            self.assertEqual(
                response.data["results"][0]["name"], "API Тест Организация"
            )
        else:
    ...
    def test_organizations_search_by_name(self):
        ...
        response = self.client.get(f"{url}?name=API&api_key={self.valid_api_key}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_activity_level_validation(self):
        ...
        level3_activity = Activity.objects.create(
            name="Говядина", parent=self.child_activity  # child_activity уже 2 уровень
        )
        self.assertEqual(level3_activity.level, 3)

        # Попытка создать деятельность 4 уровня должна вызвать ошибку
        with self.assertRaises(ValueError):
            Activity.objects.create(name="Стейки", parent=level3_activity)
